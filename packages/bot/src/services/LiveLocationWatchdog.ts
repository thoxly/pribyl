...
/**
 * In‑memory representation of a live‑location watch. MongoDB хранит ту же
 * информацию для переживания рестартов и горизонтального масштабирования.
 */
type UserId = string; // ObjectId.toString()
interface Watch {
  messageId: number;
...
// ────────────────────────────────────────────────────────────────────────────
(async function warmUpLiveWatches(): Promise<void> {
  try {
    const rows = await LiveWatchRepo.getActive();
    ...
    logger.info(
      `[Watchdog] 🔁 Restored ${rows.length} active watches from Mongo (startup)`
    );
  } catch (e) {
    logger.error(`[Watchdog] ❌ Warm‑up error: ${(e as Error).stack ?? e}`);
  }
})();
...
export async function startWatch(
  userId: Types.ObjectId,
  messageId: number,
  msgDate: number,      // unixtime от Telegram
  livePeriod: number    // секунд
): Promise<void> {
  const id = userId.toString();
  const expireAt = (msgDate + livePeriod) * 1000;
  ...
  await LiveWatchRepo.upsert(userId, messageId, expireAt);
...
export async function updateWatch(
  ...
  if (!w && hasLivePeriod) {
    logger.debug(`[Watchdog] ♻️ Self‑healing: create watch on the fly for ${id}`);
    await startWatch(userId, 0, Math.floor(Date.now() / 1000), 30 * 60); // fallback 30 мин
    w = WATCHES.get(id) as Watch;
  }
  ...
  w.lastSeen = Date.now();
  await LiveWatchRepo.touch(userId);
...
export function stopWatch(userId: Types.ObjectId): Promise<void> {
  logger.debug(`[Watchdog] ⛔ stopWatch called for userId=${userId}`);
...
async function endSharing(id: UserId): Promise<void> {
  WATCHES.delete(id);
  await LiveWatchRepo.remove(new Types.ObjectId(id));
  ...
  try {
    ...
    logger.error(`[Watchdog] ❌ Error while ending sharing for ${id}: ${(e as Error).stack ?? e}`);
  }
}
